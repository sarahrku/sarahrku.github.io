...
\begin{verbatim}
##  [1]  1.77327930 -1.39868048  1.28496391 -1.43666908 -1.34861423  0.29871852
##  [7] -0.84009906  0.32581624 -2.31403995 -1.40173332 -0.16225598  0.09992230
## [13]  0.80566602 -0.03205436  1.24819826  2.81087683 -0.20731606  0.20416810
## [19]  0.93581285 -1.99680321  2.30205774 -0.06403052  0.28079686  0.08758140
## [25]  0.09636265
\end{verbatim}
...
\DocumentationTok{\#\# [1]  1.7732793 {-}1.3986805  1.2849639 {-}1.4366691 {-}1.3486142  0.2987185}
\FunctionTok{tail}\NormalTok{(x) }\CommentTok{\#sidste 6}
\DocumentationTok{\#\# [1] {-}1.99680321  2.30205774 {-}0.06403052  0.28079686  0.08758140  0.09636265}
\NormalTok{x[}\DecValTok{1}\NormalTok{] }\CommentTok{\#første værdi}
\DocumentationTok{\#\# [1] 1.773279}
\NormalTok{x[}\FunctionTok{length}\NormalTok{(x)] }\CommentTok{\#sidste data point}
\DocumentationTok{\#\# [1] 0.09636265}
\end{Highlighting}
\end{Shaded}
...
\begin{verbatim}
## [1]  0.05407699  0.09636265  2.81087683 -2.31403995  1.63645384  1.27923955
\end{verbatim}
...
## -2.31404 -0.84010  0.09636  0.05408  0.80567  2.81088
\end{verbatim}
...
\textbf{8)} (\textbf{indlæse data}) Åbn en fil, der sidder i Absalon og hedder ``reactions.txt'' ved at bruge funktionen \texttt{read.table()} (kalde det for \texttt{data}). Husk at tjekke, om selve filen har variabelnavne og bruge således \texttt{header=T} hvis nødvendigt.
...
\textbf{9)} (\textbf{factor variabler}) Variablerne \texttt{subject} og \texttt{time} indlæses som henholdvis data type `int' (heltal) og ``chr'' (character) men de skal hellere være `factor' variabler. Lav dem om til facktor variabler.
...
\textbf{10)} (\textbf{delmængde af dataframe})
...
\textbf{11)} (\textbf{mean og tapply}) Benyt funktionen \texttt{mean} til at beregne den gennemsnitlige reaktionstid (variablen \texttt{RT}) til ``before'' og ``after'' hver for sig (brug ovenstående delmængder).
...
\textbf{12)} (\textbf{beregn forskellen og mean})
...
\textbf{13)} (\textbf{lav t-test i R}) Lav en t-test (funktionen \texttt{t.test}) for at teste hypotesen at den gennemsnitslige forskel i reaktionstiderne mellem ``before'' og ``after'' er anderledes end 0.
...
\textbf{14)} Skriv en kort sætning med din konklusion.
...
\textbf{15)} (\textbf{Chi-sq}) Kør følgende kode til at få en tabel (selve koden er ikke vigtigt):
...
\textbf{16)} (\textbf{Korrelation analyse}) Åbn datasættet \texttt{trees} og lav et scatter plot med variablen \texttt{Girth} på x-aksen og variablen \texttt{Volume} på y-aksen.
...
\textbf{17)} (\textbf{ANOVA}) OBS: hvis du føler dig utryg med funktionen \texttt{lm()} - der kommer en video om det i morgen (i forbindelse med emnet Rmarkdown).
...
\textbf{18)} (\textbf{ANOVA}) Lav en lignende analyse på datasættet \texttt{chickwts} for at svare på spørgsmålet:
...
\textbf{Valgfri - vi gennemgår også lineær regression i morgen}
